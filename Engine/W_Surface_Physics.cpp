...
namespace World
{
    bool Surface::PerformActivate( Math::Vector position, Math::Vector point )
    {
	    Math::Plane plane;
	    float planeD, newD;
	    float t;
    ...
    void Surface::SurfaceCollisions( Thing* thing )
    {
	    Math::Vector offset;
	    ...
	    if( offset.Magnitude() > 0 )
	    {
		    if( !adjoined || ( doThingCollisions && flags&JK_SURFACE_IMPASSABLE ) )
		    {
			    ...
			    if( thing->GetType() == Thing::WEAPON && ( thing->GetTypeFlags() & JK_WEAPON_EXPLODE_SURFACE ) )
			    {
				    thing->Explode();
			    }
		    }
    ...
    bool Surface::FloorCollisions( Thing* thing )
    {
